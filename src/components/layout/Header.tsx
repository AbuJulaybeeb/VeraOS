...
const INITIAL_NOTIFICATIONS: NotificationItem[] = [
  ...
  },
  {
    id: "notif_3",
    type: "warning",
    title: "[Demo] Unverified Settlement Claim",
  ...
  return (
    <>
      <header className="fixed top-0 right-0 left-0 lg:left-72 h-16 bg-surface/85 backdrop-blur-xl z-40 flex items-center justify-between px-4 sm:px-space-md lg:px-space-lg border-b border-white/5 shadow-[0_1px_8px_rgba(0,0,0,0.3)]">
        ...
        {/* Right Actions */}
        <div className="flex items-center gap-1.5 sm:gap-space-sm relative">
          {/* Search — hidden on small mobile */}
          <button
            type="button"
            onClick={() => setSearchOpen(true)}
            className="hidden sm:flex items-center gap-space-sm px-space-sm py-1.5 rounded-lg bg-surface-container text-on-surface-variant hover:text-on-surface hover:bg-surface-container-high transition-colors text-left cursor-pointer"
            title="Search (⌘K)"
            aria-label="Search (⌘K)"
          >
          ...
          {/* Agent Widget — hidden below lg */}
          <div className="hidden lg:block">
            <AgentHeaderWidget />
          </div>

          {/* Stellar Wallet — hidden below xl */}
          {user?.walletAddress ? (
            <button
              type="button"
              onClick={() => setWalletModalOpen(true)}
              className="hidden xl:flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-surface-container hover:bg-surface-container-high border border-white/10 text-xs font-mono text-[#00E5FF] transition-all cursor-pointer"
              title={`Stellar Wallet: ${user.walletAddress}`}
            >
              <span className="material-symbols-outlined text-[15px] text-[#00E5FF]">account_balance_wallet</span>
            ...
            <button
              type="button"
              onClick={() => setWalletModalOpen(true)}
              className="hidden xl:flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-surface-container hover:bg-surface-container-high text-xs font-medium text-on-surface-variant hover:text-on-surface transition-all cursor-pointer border border-white/5"
          ...
          {/* Bot Invites — hidden below xl */}
          <button
            type="button"
            onClick={() => setInviteModalOpen(true)}
            className="hidden xl:flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-surface-container hover:bg-surface-container-high border border-[#E08A3E]/30 text-xs font-medium text-[#E08A3E] hover:text-[#ff9b49] transition-all cursor-pointer shadow-sm"
          ...
          {/* New Verification — always visible, text collapses on xs */}
          <Button
            variant="primary"
            size="sm"
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  <span className="hidden sm:inline text-xs font-medium text-on-surface max-w-[80px] truncate">
                    {user?.name.split(" ")[0]}
                  </span>
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="hidden sm:block px-2.5 py-1.5 rounded-lg text-xs font-medium text-on-surface-variant hover:text-on-surface hover:bg-surface-container transition-colors cursor-pointer"
                >
                  Sign In
                </button>
                <button
                ...
                <div className="absolute right-0 top-12 w-64 rounded-2xl bg-surface-container border border-white/10 shadow-2xl z-50 p-3 flex flex-col gap-2 animate-in fade-in zoom-in-95 duration-150">
                  <div className="p-2.5 rounded-lg bg-surface-container-low flex flex-col gap-1.5">
                    <div className="flex items-center gap-2.5">
                      {user?.avatar ? (
                        <img src={user.avatar} alt={user.name} referrerPolicy="no-referrer" className="w-9 h-9 rounded-full object-cover shrink-0 border border-white/15" />
                      ) : (
                      ...
                      <div className="flex flex-col min-w-0">
                        ...
                        <span className="text-[11px] text-outline font-mono truncate">{user?.email}</span>
                      </div>
                    </div>
                  </div>

                  <button onClick={() => { setProfileDropdownOpen(false); navigate("/account"); }}
                    className="w-full text-left px-2.5 py-1.5 rounded-lg text-xs text-on-surface hover:bg-surface-container-high transition-colors flex items-center gap-2 font-medium">
                    <span className="material-symbols-outlined text-[16px] text-primary-container">manage_accounts</span>
                    <span>Account & Credentials</span>
                  </button>

                  <button onClick={() => { setProfileDropdownOpen(false); setInviteModalOpen(true); }}
